...
% This is a placeholder that will need to be set to the corret camera ID
% dependent on whatever scheme is decided upon
cameraID = 0;
    ...
    function deleteLostTracks()
        ...
        if (size(lostTracks) >= 1)
            for i = 1:size(lostTracks)
                ...
                removeString = strcat('{"age":"LOST", "connectionType": "DATASOURCE",','"cameraID":',num2str(cameraID), ',' , '"id":', num2str(lostTracks(i).id), ',"origin": {"x":', num2str(x), ',"y":', num2str(y), ...
...
% an ampersand.
    ...
    function sendDataToServer()
            ...
            for k = 1:length(NewTracks)
                ...
                newString = strcat('{"age":"NEW", "connectionType": "DATASOURCE",','"cameraID":',num2str(cameraID), ',' , '"id":', num2str(NewTracks(k).id), ',"origin": {"x":', num2str(x), ',"y":', num2str(y), ...
                ',"z": 0}, "orientation": {"x":0, "y":0, "z":0, "theta":0}, "source":"MATLAB", "updateTime":0, "creationTime":0,', ...
            ...
            for k = 1:length(OldTracks)
                ...
                oldString = strcat('{"age":"OLD", "connectionType": "DATASOURCE",','"cameraID":',num2str(cameraID), ',' , '"id":', num2str(OldTracks(k).id), ',"origin": {"x":', num2str(x), ',"y":', num2str(y), ...
                ',"z": 0}, "orientation": {"x":0, "y":0, "z":0, "theta":0}, "source":"MATLAB", "updateTime":0, "creationTime":0,', ...
